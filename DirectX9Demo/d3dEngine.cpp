/******************************************************************************
			d3dEngine.cpp
			By: Austin Lott
			This file defines the methods for Engine class.
******************************************************************************/
	...
	IDirect3DDevice9** device)
{
	...
	*/
	...
	exp = new psys::Firework(&origin, 800);
...
bool Engine::Setup()
{
	...
	r = LoadBitmapToSurface(TEXT("baboon.bmp"),
		&pSurface, Device);
	if (FAILED(r)) {
...
/*
Initialized the Tiger model and matrix.
*/
int Engine::InitTiger() 
{
	LPD3DXBUFFER pD3DXMtrlBuffer;

	D3DXMatrixTranslation(&MatTiger_, 0.0f, 2.0f, -2.5f);

	// Load the mesh from the specified file
	if (FAILED(D3DXLoadMeshFromX(TEXT("Tiger.x"), D3DXMESH_SYSTEMMEM,
		Device, NULL,
